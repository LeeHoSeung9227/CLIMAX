...
def main():
    """메인 실행 함수 - 빠른 테스트용"""
    print("🚀 고도화된 지하철 혼잡도 예측 모델 (빠른 테스트)")
    print("=" * 60)
    print("빠른 테스트: 작은 샘플, 적은 튜닝, 간단한 특성선택")
    print("=" * 60)
    ...
    try:
        ...
        print(f"⚠️ 시각화 오류 (건너뛰기): {str(e)}")
    
    # 7. 모델 저장
    try:
        model.save_model('../result/enhanced_subway_model_fast.pkl')
    except:
        print("⚠️ 모델 저장 경로 문제 - 현재 폴더에 저장")
        model.save_model('enhanced_subway_model_fast.pkl')
